...
- Backend (`server/`) subscribes to `mqtts://mqtt.hsl.fi:8883` topic `/hfp/v2/journey/ongoing/vp/tram/#`, keeps an in-memory map of tram positions, evicts stale entries after 60 seconds, and relays snapshots + updates over WebSocket at `/ws`.
